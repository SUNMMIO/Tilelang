...
def matmul_persistent_dynamic(K, block_M, block_N, block_K, dtype=T.bfloat16, accum_dtype=T.float32):
    M = T.dynamic("m")
    N = T.dynamic("n")
...
def bf16_gemm_dynamic(K, block_M, block_N, block_K):
    return matmul_persistent_dynamic(
        K,
        block_M,
        block_N,
        block_K,
