...
#if 0 // Legacy heuristic model retained for reference.
...
#endif // Legacy heuristic model.

constexpr int kVectorBits = 4096;
...
int64_t GetConstantRegionElements(const BufferRegion &region,
                                  const Stmt &stmt) {
  int64_t elements = 1;
  for (const Range &range : region->region) {
    const auto *extent = range->extent.as<IntImmNode>();
    ICHECK(extent) << "Cost-model region extents must be IntImm: " << stmt;
    elements *= extent->value;
  }
  return elements;
}

int64_t GetRegionBytes(const BufferRegion &region, const Stmt &stmt) {
  int64_t elements = GetConstantRegionElements(region, stmt);
  int64_t bits = region->buffer->dtype.bits() * region->buffer->dtype.lanes();
...
MemoryKind ClassifyMemory(const Buffer &buffer) {
  std::string scope = buffer.scope();
  if (scope.empty() || scope == "global") {
    return MemoryKind::kDram;
  }
  if (scope.find("rsram") != std::string::npos) {
    return MemoryKind::kRsram;
  }
  if (scope.find("wsram") != std::string::npos) {
    return MemoryKind::kWsram;
  }
  if (scope.find("asram") != std::string::npos) {
    return MemoryKind::kAsram;
  }
  if (scope.find("tcm") != std::string::npos) {
    return MemoryKind::kTcm;
  }
  return MemoryKind::kOther;
}

struct Gem5ODMAPath {
  int first_access_latency;
  int request_bytes;
  int completion_latency;
};

Gem5ODMAPath GetODMAPath(MemoryKind src, MemoryKind dst) {
  if (src == MemoryKind::kDram && dst == MemoryKind::kRsram) {
    // dmaPreProc -> dmaEventDone traces for 2 KiB and 4 KiB transfers are
    // 72 and 74 cycles respectively. They follow the local-SRAM 67-cycle
    // startup path, not the separately configured 95-cycle lrsram value.
    return {67, 1024, 2};
  }
  if (src == MemoryKind::kDram) {
    return {67, 1024, 2};
  }
  if (src == MemoryKind::kRsram &&
      (dst == MemoryKind::kWsram || dst == MemoryKind::kAsram)) {
    return {10, 1024, 2};
  }
  if (src == MemoryKind::kTcm || dst == MemoryKind::kTcm) {
    return {12, 16, 2};
  }
  return {7, 1024, 2};
}

class Gem5VectorCycleAnalyzer : public StmtExprVisitor {
public:
  int64_t Analyze(const Stmt &stmt) {
    cycles_ = 0;
    loop_extent_ = 1;
    VisitStmt(stmt);
    return std::max<int64_t>(1, cycles_);
  }

private:
  void VisitStmt_(const ForNode *op) final {
    const auto *extent = op->extent.as<IntImmNode>();
    ICHECK(extent) << "VectorCore loop extent must be IntImm: "
                   << tvm::ffi::GetRef<Stmt>(op);
    int64_t saved_extent = loop_extent_;
    loop_extent_ *= extent->value;
    VisitStmt(op->body);
    loop_extent_ = saved_extent;
  }

  void VisitStmt_(const BufferStoreNode *op) final {
    int64_t element_bits = op->buffer->dtype.bits() * op->buffer->dtype.lanes();
    int64_t chunks =
        std::max<int64_t>(1, CeilDiv(loop_extent_ * element_bits, kVectorBits));
    int64_t saved_multiplier = instruction_multiplier_;
    instruction_multiplier_ = chunks;
    VisitExpr(op->value);
    // Five isolated issue -> complete samples are [3, 3, 3, 4, 4] cycles
    // (mean 3.4); use the nearest integral service latency.
    cycles_ += 3 * chunks;
    instruction_multiplier_ = saved_multiplier;
  }

  void VisitStmt_(const EvaluateNode *op) final {
    const auto *call = op->value.as<CallNode>();
    if (call && call->op.same_as(Op::Get("tl.vector_core_in_tile_reduce"))) {
      const auto *kind = call->args[0].as<StringImmNode>();
      ICHECK(kind) << "Vector reduce kind must be StringImm.";
      cycles_ += kind->value == "sum" ? 14 : 2;
      return;
    }
  ...
  void VisitExpr_(const BufferLoadNode *op) final {
    // Five isolated vle16 samples are [13, 14, 14, 14, 13] cycles (mean
    // 13.6), which rounds to 14 for the integral planner time base.
    cycles_ += 14 * instruction_multiplier_;
    for (const PrimExpr &index : op->indices) {
      VisitExpr(index);
    }
  }

  void VisitExpr_(const AddNode *op) final { AddBinary(op, 4); }
  void VisitExpr_(const SubNode *op) final { AddBinary(op, 4); }
  void VisitExpr_(const MulNode *op) final { AddBinary(op, 4); }
  void VisitExpr_(const MaxNode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const MinNode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const LENode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const LTNode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const GENode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const GTNode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const EQNode *op) final { AddBinary(op, 3); }
  void VisitExpr_(const NENode *op) final { AddBinary(op, 3); }

  void VisitExpr_(const CastNode *op) final {
    cycles_ += 3 * instruction_multiplier_;
    VisitExpr(op->value);
  }

  void VisitExpr_(const CallNode *op) final {
    if (op->op.same_as(Op::Get("tir.exp2"))) {
      cycles_ += 11 * instruction_multiplier_;
    } else if (op->op.same_as(Op::Get("tir.bitwise_and"))) {
      cycles_ += 3 * instruction_multiplier_;
    }
  ...
  template <typename T> void AddBinary(const T *op, int latency) {
    cycles_ += latency * instruction_multiplier_;
    VisitExpr(op->a);
    VisitExpr(op->b);
  }
  ...
  int64_t instruction_multiplier_{1};
};
...
float CostModel::EstimateTensorCoreDelay(const tir::Stmt &stmt) {
  const auto *block = stmt.as<BlockRealizeNode>();
  ICHECK(block) << "TensorCore command must be a BlockRealizeNode: " << stmt;
  const auto *eval = block->block->body.as<EvaluateNode>();
  ICHECK(eval) << "TensorCore block body must be an EvaluateNode: " << stmt;
  const auto *call = eval->value.as<CallNode>();
  ICHECK(call && call->op.same_as(Op::Get("tl.mma_sunmmio")))
      << "TensorCore command must call tl.mma_sunmmio: " << stmt;

  const auto *lhs = call->args[0].as<CallNode>();
  const auto *rhs = call->args[1].as<CallNode>();
  ICHECK(lhs && rhs && lhs->args[2].as<IntImmNode>() &&
         lhs->args[3].as<IntImmNode>() && rhs->args[3].as<IntImmNode>())
      << "TensorCore MMA dimensions must be constant: " << stmt;

  int64_t rows = lhs->args[2].as<IntImmNode>()->value;
  int64_t reduction = lhs->args[3].as<IntImmNode>()->value;
  int64_t cols = rhs->args[3].as<IntImmNode>()->value;
  int64_t work_blocks =
      CeilDiv(rows, 32) * CeilDiv(cols, 32) * CeilDiv(reduction, 32);

  // Five isolated tcStart -> Calc done samples are exactly 38 cycles for
  // 32x32x32 and 41 cycles for 32x32x128. They fit a 37-cycle pipeline cost
  // plus one cycle per native 32x32x32 work block. The configured warmup occurs
  // before tcStart and is not part of this service interval.
  return static_cast<float>(37 + work_blocks);
}
...
float CostModel::EstimateODMADelay(const tir::Stmt &stmt) {
  ...
  if (call->op.same_as(Op::Get("tl.dma_copy")) ||
    ...
    int64_t bytes = GetRegionBytes(src, stmt);
    Gem5ODMAPath path =
        GetODMAPath(ClassifyMemory(src->buffer), ClassifyMemory(dst->buffer));
    return static_cast<float>(path.first_access_latency +
                              CeilDiv(bytes, path.request_bytes) +
                              path.completion_latency);
  }

  if (call->op.same_as(Op::Get("tl.broadcast_"))) {
    BufferRegion src = NormalizeToBufferRegion(call->args[kBroadcastArgSrc]);
    int64_t bytes = GetRegionBytes(src, stmt);
    // gem5 remote first-access latency plus 512-byte link request granularity.
    return static_cast<float>(52 + CeilDiv(bytes, 512));
  }
...
float CostModel::EstimateVectorCoreDelay(const tir::Stmt &stmt) {
  Gem5VectorCycleAnalyzer analyzer;
  return static_cast<float>(analyzer.Analyze(stmt));
}
