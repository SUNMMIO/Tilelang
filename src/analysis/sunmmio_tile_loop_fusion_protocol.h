...
// Shared protocol for the Sunmmio tile loop fusion pipeline.
//
...
//     -> rewritten PrimFunc
//
// Discovery owns the program and window-problem summaries.
// Planner owns choosing an order and shared execution shells per problem.
// Rewrite owns re-emitting TIR from the original program plus planner plans.
//
// This header only contains data that is exchanged across stage boundaries.
// Planner search state and rewrite-local helper state belong in the stage-local
// implementation files instead.
...
// One planner-visible lowered region.
//
// Why this exists:
// Discovery needs a stable, immutable record for each `tile.scope_entry` region
// so later stages never have to rediscover wrapper structure or execution-loop
// metadata from raw TIR. This is the canonical region identity used by the rest
// of the pipeline.
...
// One maximal consecutive source-order run of planner-visible regions.
//
// Why this exists:
...
// One normalized buffer dimension plus its logical-axis provenance.
//
// Why this exists:
...
// One logical-axis-normalized external access.
//
// Why this exists:
...
// One legality/profitability dependence between two discovered regions.
//
// Why this exists:
...
struct TileScopeDependenceEdge {
  ...
  // Exact overlap fragment when analysis can construct one. This is kept only
  // for explain/debug consumers and is not the planner identity for the edge.
  std::optional<tir::BufferRegion> debug_overlap_region;
};

// Dependence graph for one planner window.
//
// Why this exists:
...
// Logical-axis-normalized boundary information for one discovered region.
//
// Why this exists:
...
// Full discovery result for one PrimFunc.
//
// Why this exists:
...
// Self-contained planning problem for one discovered window.
//
// Why this exists:
...
// Planner trace record for one scheduled region.
//
// Why this exists:
// The exact DP internally reasons in terms of close/open shell actions. The
// resulting trace is retained because current tests and debug summaries inspect
// it directly, even though rewrite ultimately consumes the derived tree below.
...
// Rewrite-ready shared-shell tree for one planned window.
//
// Why this exists:
// Rewrite should consume a structural description of fused shells rather than
...
// Final planner result for one window problem.
//
// Why this exists:
// This is the contract between planning and rewrite. The tree is the production
// payload; `order` and `actions` are retained as debug projections for schedule
// regression tests and analysis summaries.
struct SunmmioTileLoopFusionWindowPlan {
  ...
  // Chosen global region order within that window.
  std::vector<int> order;
  // Final lexicographic planner score for the chosen schedule.
  SunmmioTileLoopFusionPlannerScore score;
  // Action trace reconstructed from the DP backpointers. Kept for debugability.
