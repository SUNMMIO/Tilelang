...
def main(A_handle: T.handle):
    ...
    with T.block("tilelang_root"):
        ...
        T.writes()
        A_shared = T.alloc_buffer((128, 128), "float16", scope="shared.rsram")
        B_shared = T.alloc_buffer((128, 128), "float16", scope="shared.rsram")
        C_shared = T.alloc_buffer((16, 128, 128), "float16", scope="shared.rsram")
        T.copy(T.region(A[by * 128, bx * 128], 1, 128, 128), T.region(A_shared[0, 0], 2, 128, 128), -1, T.bool(False), 0)
...
class Module:
    ...
    def main(A_handle: T.handle):
        ...
        with T.block("tilelang_root"):
            ...
            A_shared = T.alloc_buffer((128, 128), "float16", scope="shared.rsram")
            B_shared = T.alloc_buffer((128, 128), "float16", scope="shared.rsram")
            T.dma_copy(T.region(A[by * 128, bx * 128], 1, 128, 128), T.region(A_shared[0, 0], 2, 128, 128))
...
class Module:
    ...
    def main(A_handle: T.handle):
        ...
        with T.block("root"):
            T.reads()
            T.writes()
            A_shared = T.Buffer((8192,), scope="local")
            T.block_attr({"layout_map": {A_shared: metadata["tl.Fragment"][0]}})
            ...
            with T.block("tilelang_root"):
                ...
                T.writes()
                T.block_attr({"layout_map": {A_shared: metadata["tl.Fragment"][0]}})
                A_shared = T.alloc_buffer((8192,), data=A_shared.data, scope="local")
                E_shared = T.alloc_buffer((1024,), scope="local")
