...
def test_sunmmio_tile_loop_fusion_planner_large_window_falls_back_to_source_order():
    ...
    assert summary["window_lengths"] == [19]
...
def test_sunmmio_tile_loop_fusion_planner_dense_flash_attention_online_softmax_chain():
    ...
    summary = get_debug_summary(mod)
    (graph,) = summary["graphs"]
    (plan,) = summary["plans"]
    reduce_max_region = summary["regions"][2]
    reduce_sum_region = summary["regions"][6]

    assert summary["region_count"] == 10
    assert summary["window_count"] == 1
    assert summary["window_lengths"] == [10]
    assert {
    ...
    assert plan["score"] == {
        "write_cut_cost": 1792,
        "shared_read_cost": 13216,
        "live_range_penalty": 26624,
        "reorder_penalty": 2,
    }
    assert [region["execution_loop_extents"] for region in summary["regions"]] == [
        ["1"],
        ["1"],
        ["8", "1"],
        ["1"],
...
def test_sunmmio_tile_loop_fusion_planner_attention_sink_keeps_late_row_update_under_outer_shell():
    ...
    assert summary["region_count"] == 11
    assert summary["window_count"] == 1
    assert summary["window_lengths"] == [11]
    assert {
        (5, 8, "RAW", "acc_s", 2, 1024),
        (6, 7, "RAW", "scores_sum", 1, 32),
...
def test_sunmmio_tile_loop_fusion_planner_nsa_chain_reorders_without_row_merge_loop():
    ...
    assert summary["region_count"] == 9
    assert summary["window_count"] == 1
    assert summary["window_lengths"] == [9]
    assert {
        (0, 1, "WAR", "scores_max", 1, 0),
        (0, 3, "RAW", "scores_max_prev", 1, 256),
...
def test_sunmmio_tile_loop_fusion_planner_rms_norm_explicit_reduce_forms_expected_outer_and_inner_shells():
    ...
    assert summary["window_count"] == 1
    assert summary["window_lengths"] == [5]
...
def test_sunmmio_tile_loop_fusion_planner_rms_norm_fill_and_rsram_copy_init_surface_init_ops_and_form_row_shells():
    ...
    fill_summary = get_debug_summary(apply_sunmmio_tiles_lowering(fill_mod))
    copy_summary = get_debug_summary(apply_sunmmio_tiles_lowering(copy_mod))

    (fill_graph,) = fill_summary["graphs"]
    (fill_plan,) = fill_summary["plans"]
    (copy_graph,) = copy_summary["graphs"]
    (copy_plan,) = copy_summary["plans"]

    assert fill_summary["region_count"] == 5
    assert fill_summary["window_count"] == 1
    assert fill_summary["window_lengths"] == [5]
    ...
    assert copy_summary["region_count"] == 6
    assert copy_summary["window_count"] == 1
    assert copy_summary["window_lengths"] == [6]
    ...
    fill_row_sum_regions = [region for region in fill_summary["regions"] if "row_sum" in buffer_regions_by_name(region["def_out"])]
    copy_row_sum_regions = [region for region in copy_summary["regions"] if "row_sum" in buffer_regions_by_name(region["def_out"])]
    copy_row_zero_regions = [region for region in copy_summary["regions"] if "row_zero" in buffer_regions_by_name(region["def_out"])]
