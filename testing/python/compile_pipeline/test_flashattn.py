...
def test_flashattn():
    ...
    script_device_mode = [
        ...
        with T.launch_thread("blockIdx.x", 16) as bx:
            with T.decl_buffer((128, 128), "bfloat16", data=Q_shared.data, scope="shared.asram") as Q_shared:
                ...
                T.sync_null_token(11)
                T.sync_null_token(13)
                for w in range(32):
                    T.dma_copy(T.region(Q_1[T.Div(T.truncmod(w * 16 + bx, 128), 64), T.Div(w * 16 + bx, 128) * 1024 + T.truncmod(bx, 8) * 128, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 1, 1, 128, 1, 128), T.region(Q_layout_stage[0, 0, 0, 0], 2, 1, 128, 1, 128), 0, T.sync_token_id(0))
                    T.wait_token(0)
                    ...
                    T.dma_copy(T.region(K_1[T.Div(T.truncmod(w * 16 + bx, 128), 64), 0, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 1, 1, 128, 1, 128), T.region(K_shared[0, 0, 0], 2, 1, 128, 128), 0, T.sync_token_id(2))
                    ...
                    T.wait_token(11)
                    T.dma_copy(T.region(V_1[T.Div(T.truncmod(w * 16 + bx, 128), 64), 0, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 1, 1, 128, 1, 128), T.region(V_shared[0, 0, 0], 2, 1, 128, 128), 0, T.sync_token_id(3))
                    T.sync_null_token(18)
                    ...
                    for k in range(31):
                        ...
                        T.dma_copy(T.region(K_1[T.Div(T.truncmod(w * 16 + bx, 128), 64), k * 128 + 128, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 1, 1, 128, 1, 128), T.region(K_shared[0, 0, 0], 2, 1, 128, 128), 0, T.sync_token_id(18))
                        for i in T.serial(1, annotations={"tile.domain": [128], "tile.execution_axis": 0, "tile.execution_domain_axes": [0], "tile.scope_entry": 1, "tile.tile_size": [128]}):
                            for ki in T.vectorized(128, annotations={"tile.interior": 1, "tile.interior_axis": 0}):
                        ...
                        T.dma_copy(T.region(acc_s[0, 0, 0], 1, 1, 128, 128), T.region(acc_s_cast[0, 0, 0], 2, 1, 128, 128), 0, T.sync_token_id(19))
                        for i in T.serial(16, annotations={"tile.domain": [128, 128], "tile.execution_axis": 0, "tile.execution_domain_axes": [0, 1], "tile.scope_entry": 1, "tile.tile_size": [8, 32]}):
                            for j in T.serial(4, annotations={"tile.execution_axis": 1}):
                                for ki in T.serial(8, annotations={"tile.interior": 1, "tile.interior_axis": 0}):
                                    for kj in T.vectorized(32, annotations={"tile.interior": 1, "tile.interior_axis": 1}):
                        ...
                        T.wait_token(23)
                        T.wait_token(19)
                        T.mma_sunmmio(T.region(acc_s_cast[0, 0, 0], 1, 1, 128, 128), T.region(V_shared[0, 0, 0], 1, 1, 128, 128), T.region(acc_o[0, 0], 3, 128, 128), T.bool(False), T.bool(False), T.bool(False), 0, T.sync_token_id(20))
                        T.wait_token(20)
                        T.dma_copy(T.region(acc_s[0, 0, 0], 1, 1, 128, 128), T.region(acc_s_cast[0, 0, 0], 2, 1, 128, 128), 1024, T.sync_token_id(21))
                        T.wait_token(21)
                        T.mma_sunmmio(T.region(acc_s_cast[0, 0, 0], 1, 1, 128, 128), T.region(V_shared[0, 0, 0], 1, 1, 128, 128), T.region(acc_o[0, 0], 3, 128, 128), T.bool(False), T.bool(False), T.bool(False), 1024, T.sync_token_id(22))
                        for i in T.serial(16, annotations={"tile.domain": [128, 128], "tile.execution_axis": 0, "tile.execution_domain_axes": [0, 1], "tile.scope_entry": 1, "tile.tile_size": [8, 32]}):
                            for j in T.serial(4, annotations={"tile.execution_axis": 1}):
                                for ki in T.serial(8, annotations={"tile.interior": 1, "tile.interior_axis": 0}):
                                    for kj in T.vectorized(32, annotations={"tile.interior": 1, "tile.interior_axis": 1}):
                                        acc_s[0, i * 8 + ki, j * 32 + kj] = T.bfloat16(0.0)
                        T.wait_token(22)
                        T.dma_copy(T.region(V_1[T.Div(T.truncmod(w * 16 + bx, 128), 64), k * 128 + 128, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 1, 1, 128, 1, 128), T.region(V_shared[0, 0, 0], 2, 1, 128, 128), 0, T.sync_token_id(23))
                    ...
                    for i in T.serial(16, annotations={"tile.domain": [128, 128], "tile.execution_axis": 0, "tile.execution_domain_axes": [0, 1], "tile.scope_entry": 1, "tile.tile_size": [8, 32]}):
                        for j in T.serial(4, annotations={"tile.execution_axis": 1}):
                            ...
                            if j == 3:
                                T.vector_core_in_tile_reduce("sum", T.region(scores_sum[i * 8], 2, 8), T.region(scores_sum_acc[0, 0], 1, 8, 32), 1)
                    T.dma_copy(T.region(acc_s[0, 0, 0], 1, 1, 128, 128), T.region(acc_s_cast[0, 0, 0], 2, 1, 128, 128), 0, T.sync_token_id(8))
                    for i in T.serial(16, annotations={"tile.domain": [128, 128], "tile.execution_axis": 0, "tile.execution_domain_axes": [0, 1], "tile.scope_entry": 1, "tile.tile_size": [8, 32]}):
                        for j in T.serial(4, annotations={"tile.execution_axis": 1}):
                            for ki in T.serial(8, annotations={"tile.interior": 1, "tile.interior_axis": 0}):
                                for kj in T.vectorized(32, annotations={"tile.interior": 1, "tile.interior_axis": 1}):
                    ...
                    T.wait_token(8)
                    T.wait_token(23)
                    T.mma_sunmmio(T.region(acc_s_cast[0, 0, 0], 1, 1, 128, 128), T.region(V_shared[0, 0, 0], 1, 1, 128, 128), T.region(acc_o[0, 0], 3, 128, 128), T.bool(False), T.bool(False), T.bool(False), 0, T.sync_token_id(9))
                    T.wait_token(9)
                    T.dma_copy(T.region(acc_s[0, 0, 0], 1, 1, 128, 128), T.region(acc_s_cast[0, 0, 0], 2, 1, 128, 128), 1024, T.sync_token_id(10))
                    T.wait_token(10)
                    T.mma_sunmmio(T.region(acc_s_cast[0, 0, 0], 1, 1, 128, 128), T.region(V_shared[0, 0, 0], 1, 1, 128, 128), T.region(acc_o[0, 0], 3, 128, 128), T.bool(False), T.bool(False), T.bool(False), 1024, T.sync_token_id(11))
                    for i in T.serial(16, annotations={"tile.domain": [128, 128], "tile.execution_axis": 0, "tile.execution_domain_axes": [0, 1], "tile.scope_entry": 1, "tile.tile_size": [8, 32]}):
                        for j in T.serial(4, annotations={"tile.execution_axis": 1}):
                            for ki in T.serial(8, annotations={"tile.interior": 1, "tile.interior_axis": 0}):
                                for kj in T.vectorized(32, annotations={"tile.interior": 1, "tile.interior_axis": 1}):
                    ...
                    T.wait_token(13)
                    T.sunmmio_layout_transform(T.region(O_shared[0, 0], 1, 128, 128), T.region(Output_layout_stage[0, 0], 2, 128, 128), T.sync_token_id(12))
                    T.wait_token(12)
                    T.dma_copy(T.region(Output_layout_stage[0, 0], 1, 128, 128), T.region(Output_1[T.Div(T.truncmod(w * 16 + bx, 128), 64), T.Div(w * 16 + bx, 128) * 1024 + T.truncmod(bx, 8) * 128, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 2, 1, 128, 1, 128), 0, T.sync_token_id(13))
            T.wait_token(13)
        return 0
        """,
    ]
    ...
    script_lower_tile_op = [
        ...
        """,
        "for w in range(32):",
        "T.dma_copy(T.region(Q[T.Div(T.truncmod(w * 16 + bx, 128), 64), T.Div(w * 16 + bx, 128) * 1024 + T.truncmod(bx, 8) * 128, T.Div(T.truncmod(w * 16 + bx, 64), 8), 0], 1, 1, 128, 1, 128), T.region(Q_layout_stage[0, 0, 0, 0], 2, 1, 128, 1, 128), 0)",
    ...
    # compile_test(func, out_idx=[2], target="Sunmmio", log_pass_output=True, log_dir=os.path.join(os.path.dirname(__file__), "_debug", "flashattn"), remove_header=True)
