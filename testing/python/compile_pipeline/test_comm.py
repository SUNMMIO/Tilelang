...
def test_comm():
    ...
    script_comm = [
        ...
        "T.broadcast_(T.region(A_remote_1[0, 0], 1, 1024, 1024), T.region(A_remote_1[0, 0], 2, 1024, 1024), 0, T.int64(61440), 12, 0)",
        "T.broadcast_(T.region(B_shared[0, 0], 1, 1024, 1024), T.region(B_remote_1[0, 0], 2, 1024, 1024), 1, T.int64(1024), 6, 0)",
        "T.broadcast_(T.region(C_shared[0, 0], 1, 1024, 1024), T.region(C_allgather_1[0, 0, 0], 2, 1, 1024, 1024), 0, T.int64(15), 0, 0)",
        "T.broadcast_(T.region(C_allgather_1[12, 0, 0], 1, 4, 1024, 1024), T.region(C_allgather_1[12, 0, 0], 2, 4, 1024, 1024), 1, T.int64(34952), 15, 0)",
        "T.broadcast_(T.region(C_shared[0, 0], 1, 1024, 1024), T.region(C_allgather_2[3, 0, 0], 2, 1, 1024, 1024), 0, T.int64(61440), 15, 0)",
        "T.broadcast_(T.region(C_shared[0, 0], 1, 1024, 1024), T.region(C_allgather_3[3, 0, 0], 2, 1, 1024, 1024), 1, T.int64(34952), 15, 0)",
    ]
    test_config = {
        "LowerTileOp": {
