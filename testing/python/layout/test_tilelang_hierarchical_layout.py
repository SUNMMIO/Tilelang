...
from tilelang.layout import HierarchicalLayout, make_hierarchical_layout, make_blockwise_zz_layout
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([2, 4, 6], [24, 6, 1], [(0, 3)], (48,), "HierarchicalLayout(hdims=((2, 4, 6)), hstrides=((24, 6, 1)))"),
        ([2, 4, 6], [24, 6, 1], [(0, 3)], (48,), "HierarchicalLayout(hdims=((2, 4, 6)), hstrides=((24, 6, 1)))"),
        (
            [2, 2, 2, 2],
            [8, 2, 4, 1],
...
def test_hierarchical_layout_properties(hdims, hstrides, groups, expected_logical_shape, expected_str):
...
def test_hierarchical_layout_index_conversion(hdims, hstrides, groups, logical_indices, expected_hierarchical_indices):
    layout = HierarchicalLayout(hdims, hstrides, groups)
...
@pytest.mark.parametrize(
    "hdims, hstrides, groups, hierarchical_indices, expected_logical_indices",
    [
        (
            [8, 128, 8, 128],
            [1024, 1, 8192, 128],
...
def test_hierarchical_layout_inverse_index_conversion(hdims, hstrides, groups, hierarchical_indices, expected_logical_indices):
    layout = HierarchicalLayout(hdims, hstrides, groups)
...
def test_make_blockwise_zz_layout(
    buffer: tuple[int, int], block_size: tuple[int, int], hdims: list[int], hstrides: list[int], groups: list[tuple[int, int]]
):
...
def test_make_blockwise_zz_layout_assertion_error():
    with pytest.raises(AssertionError, match="Row and column must be multiples of block sizes for blockwise ZZ layout."):
